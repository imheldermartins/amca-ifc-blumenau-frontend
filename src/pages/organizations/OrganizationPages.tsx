...
import { Button, TextField, Select } from 'cubs-components'
...
export function OrganizationsPage() {
  const {lang}=useParams({strict:false});const {user}=useAuth();const navigate=useNavigate()
  const organizations=useQuery({queryKey:['organizations',user?.id],queryFn:()=>workspaceService.listOrganizations()})
  return <main className="min-h-dvh bg-background px-6 py-10 text-foreground"><section className="mx-auto max-w-4xl">
    <Button variant="text" color="from-theme" onClick={()=>navigate({href:'/'+lang+'/workspaces?choose=true'})}>{i18n('organization.workspaces-back')}</Button>
    <header className="mt-8 flex flex-wrap items-end justify-between gap-5 border-b border-divider pb-6"><div>
      <Typography variant="h1">{i18n('organization.title')}</Typography><Typography variant="body" as="p" className="mt-2 text-foreground/60">{i18n('organization.subtitle')}</Typography>
    </div><Button variant="filled" color="purple" onClick={()=>navigate({href:'/'+lang+'/organizations/new'})}>{i18n('organization.create')}</Button></header>
    {organizations.isPending?<Feedback/>:organizations.isError?<Feedback error/>:<ul className="divide-y divide-divider">
      {organizations.data.map(org=><li key={org.id} className="flex items-center justify-between gap-4 py-6">
        <div><Typography variant="h3">{org.name}</Typography><Typography variant="caption" as="p" className="mt-1 text-foreground/60">{org.isOwner?i18n('access.owner'):org.roleName||i18n('access.member')}{can(org,'read','workspaces')?' · '+i18n('organization.workspace-count',{count:org.workspaceCount}):''}</Typography></div>
        <Button variant="text" color="purple" onClick={()=>navigate({href:'/'+lang+'/organizations/'+org.id})}>{i18n('organization.open')}</Button>
      </li>)}{!organizations.data.length&&<Feedback message={i18n('organization.empty')}/>}
    </ul>}
  </section></main>
}
export function NewOrganizationPage() {
  ...
  const create=useMutation({mutationFn:(value:{name:string})=>workspaceService.createOrganization(value),
  ...
  return <main className="min-h-dvh bg-background px-5 py-10 text-foreground"><section className="mx-auto max-w-xl">
    <Button variant="text" color="from-theme" onClick={()=>navigate({href:'/'+lang+'/organizations'})}>{i18n('access.back')}</Button>
